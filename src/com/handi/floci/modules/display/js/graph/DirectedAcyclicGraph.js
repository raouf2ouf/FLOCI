function DirectedAcyclicGraph() { // is a selection, not a simple object, but a selectable node in the document DOM!
     ...
     */
    function graph(selection) { // selection is a set of Graphs
        selection.each(function(data) { // data is a Graph object  
            // Select the g element that we draw to, or add it if it doesn't exist
            var svg = d3.select(this).selectAll("svg").data([data]);
            svg.enter().append("svg").attr("class", "backgroundsvg").append("g").attr("class", "graph").classed("animate", animate);
            
            // Size the chart
            svg.attr("width", width.call(this, data)); // default sets the width to 100%
            svg.attr("height", height.call(this, data));  // default sets the height to 100%          
            
            // Get the edges and nodes from the data.  Can have user-defined accessors
            var edges = getedges.call(this, data); // get visible links from the Graph data (array)
            var nodes = getnodes.call(this, data); // get visible nodes from the Graph data (array)
            
            // Get the existing nodes and edges, and recalculate the node size
            var existing_edges = svg.select(".graph").selectAll(".edge").data(edges, edgeid); // egdeid(edge) creates a key for every edge, this allows data() to know if an edge already exists even if it changes places in the edges array
            ...
            var new_edges = existing_edges.enter().insert("path", ":first-child").attr("class", "edge entering"); // why intsert new edges? why not just append them
            ...
            if (animate) {
                ...
                svg.select(".graph").selectAll(".edge.visible").attr("d", graph.splineGenerator); // generate the line     
                existing_nodes.attr("transform", graph.nodeTranslate); // reposition existing nodes        
            }
    ...
    var nodename = function(d) { return d.id; }
    ...
    var drawnode = function(d) { // This is what you want to change if you want to draw circles or change label position
    ...
    var layout = function(nodes_d, edges_d) {
        ...
        nodes_d.forEach( function(node) { g.setNode(node.id, node); });
        edges_d.forEach( function(edge) { g.setEdge(edge.source.id, edge.target.id); });

        dagre.layout(g);
        console.log("layout:dagre", (new Date().getTime() - start));   
        
        // Put the claculated edges points into the edges joined data
        d3.select(this).selectAll(".edge").each(function(d) {
            var edge = g.edge(d.source.id, d.target.id);
            d.points = edge.points;
        });
        ...
        d3.select(this).selectAll(".node.pre-existing").each(function(d) {
            if (d.dagre_prev) {
                count++;
                x += (d.dagre_prev.x - d.dagre.x); // the sum of the difference between the old pos and the new pos of all pre-existing nodes
                y += (d.dagre_prev.y - d.dagre.y);
            }
        });
        if (count > 0) { // there are some existing nodes
            x = x / count; // the average difference from old pos and new pos
            y = y / count;
            d3.select(this).selectAll(".node").each(function(d) {
                d.dagre.x += x; // add that average difference to the new coordinate to make the whole graph close to the old position
                d.dagre.y += y;
            })
            d3.select(this).selectAll(".edge").each(function(d) {
                d.dagre.points.forEach(function(p) { // modify every edge's point
                    p.x += x;
                    p.y += y;
                })
            })
     ...
     */
    graph.splineGenerator = function(d) { // generates the line to draw
        return d3.svg.line().x(function(d) { return d.x }).y(function(d) { return d.y }).interpolate("basis")(edgepos.call(this, d));
        /*return d3.svg.diagonal.radial().projection(function(d) { return [d.y, d.x / 180 * Math.PI]; })(edgepos.call(this, d));*/
        /*return d3.svg.line().x(function(d) { return d.y }).y(function(d) { return d.x / 180 * Math.PI }).interpolate("basis")(edgepos.call(this, d));*/
    }
